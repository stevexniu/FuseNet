...
CombinePlots(plots = list(DimPlot(cbmc, reduction = "umap", group.by = "rnaClusterID", label = TRUE) + ggplot2::ggtitle("RNA-seq Data"), DimPlot(cbmc1, reduction = "tsne_adt", group.by = "rnaClusterID", label = TRUE) + ggplot2::ggtitle("ADT Surface Protein Data")))
```
...
ptm <- proc.time()
cbmc_rna <- RunWLR(object = cbmc_rna, n_iters = 1000, wlr_pca_dims = 20, wlr_k = 30, ratio = 0.1, norm_type = "l1")
ptm1 <- proc.time() 
print(x = ptm1 - ptm)
```
...
ptm <- proc.time()
cbmc_prt <- RunWLR(object = cbmc_prt, n_iters = 1000, ratio = 0.1, wlr_k = 30, norm_type = "l1")
ptm1 <- proc.time() 
print(x = ptm1 - ptm)
```

4. We fuse the RNA and ADT data. We see that CD4+ and CD8+ T cells are more defined by the ADT data in the surface protein space, while Dendritic cells (DCs) are more defined in the RNA-seq data due to the absence of its surface marker in the ADT data.
```{r fuse, fig.height=5, fig.width=10}
fused <- FuseWLR(cbmc_rna, cbmc_prt, project_k = 10)
cbmc@meta.data["RNA_Score"] <- fused$fused_weight["RNA",]
cbmc@meta.data["Protein_Score"] <- fused$fused_weight["Protein",]
CombinePlots(plots = list(VlnPlot(object = cbmc1, features = "RNA_Score", sort = TRUE), VlnPlot(object = cbmc1, features = "Protein_Score", sort = TRUE)))
```
